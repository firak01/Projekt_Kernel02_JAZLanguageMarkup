...
<xsl:stylesheet version="1.0" 
	...
	<!-- Standard java HashMap reicht mir aber fuer die Loesung nicht: <xsl:param name="mapTableHeadLabel" select="java:java.util.HashMap.new" /> -->
	<xsl:param name="mapTableHeadLabelIndexed" select="java:zBasic.util.abstractList.HashMapIndexedZZZ.new" />
		...
		<table id="tableWithIpNr">
			...
			<tr>
				...
				<xsl:for-each select="column">
					...
					<xsl:message><xsl:value-of select="concat(string('Header: Index from java counter='),string($index))"/></xsl:message>
...
<!-- 					</th> -->
					...
					<xsl:message><xsl:value-of select="concat(string('Header: TheHeaderId by Index='),string($headId_byIndex))"/></xsl:message>
					...
					<xsl:message><xsl:value-of select="concat(string('Header: TheHeaderLabel by Index='),string($headLabel_byIndex))"/></xsl:message>
...
<!-- Loesung ueber eine HashMap mit Indizierung. Der Index gibt die Reihenfolge der Spalten an. -->
						...
						<xsl:message><xsl:value-of select="concat(string('Data: The HeadId by Index='),string($headId_byIndex))"/></xsl:message>

<!--         Beispiels fuer Zugriff auf Attribute -->	
...
<!--         </xsl:for-each> -->
						...
						<!-- Beispiel: Zugriff auf ein Element, dass auf der gleichen Ebene liegt, wie das Element, welches den Wert 17 hat -->
						...
						<xsl:message><xsl:value-of select="concat(string('Data: The Value by HeadId byIndex='),string($value_byHeadId))"/></xsl:message>
...
<!-- VERALTETES TEMPLATES, Aus Dokumentationsgruenden stehen lassen, beschreiben  -->
