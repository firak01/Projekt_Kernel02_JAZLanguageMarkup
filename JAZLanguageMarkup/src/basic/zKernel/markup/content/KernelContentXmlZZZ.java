...
public abstract class KernelContentXmlZZZ extends AbstractKernelUseObjectZZZ implements IKernelContentZZZ, IKernelContentMultiZZZ, IKernelContentXmlZZZ, IKernelContentComputableZZZ{
	...
	* lindhauer; 22.02.2008 07:06:39
	 ...
	 * 
	 * F�gt den Wert an der Stelle hinzu
	 */
			...
			if(!list.isEmpty()){
				...
				for(int icount = iIndex; icount <= iListSizeAtStart - 1; icount++){
					...
					if(icount == iIndex){
						...
						elemDummy.detach();//Wichtig, sonst kann man es nicht mit addConent() hinzuf�gen
						elemRoot.addContent(elemDummy);   //Die in der vorherigen Schleifenrunde gesicherte Dummy Variable ganz ans Ende anh�ngen
	...
	public boolean setVarAsFirst(String sVariableName, String sVariableValue) throws ExceptionZZZ {
		 ...
		 * 
		 * Dies erzeugt einen XML-String eine Ebene h�her als das Root, falls dort noch kein Element mit dem Namen vorhanden ist.
	...
	//######### DOCUMENT ###########################################################
		/* F�ge das �bergebene Xml-Dokument an das intern verwaltete Dokument an.
		 * Der Root des �bergebenen Xml-Dokuments wird dabei in den Variablennamen umbenannt.
		 * 
		 * 
		 * 
		 * (non-Javadoc)
		...
		public boolean setVar(String sVariableName, Document objDocJDom) throws ExceptionZZZ{
			...
			main:{
				...
				//######################################
				//1. Umbenennen des Roots vom hinzuzuf�genden Dokument. Unter diesem Namen wird man wieder darauf zugreifen k�nnen.
				elemRoot2Add.setName(sVariableName);
				
				Document document = this.getDocument();
				Element elemRoot = document.getRootElement();
				
				//Umh�ngen des Elements von einem Dokument zu einem anderen !!!
		...
		public Document getVarDocument(String sVariableName)throws ExceptionZZZ{
			...
			main:{
				...
				//Neues Dokument f�r die R�ckgabe erstellen und das bisherige Unterelement als Rootelement anh�ngen
	...
	* lindhauer; 25.02.2008 09:39:58
	...
	public boolean toFile(File objFile) throws ExceptionZZZ{
		...
		main:{
			...
			try{
				//FileWriter objWriter = new FileWriter(objFile, true); //true = anh�ngen
				FileWriter objWriter = new FileWriter(objFile); //true = anh�ngen
