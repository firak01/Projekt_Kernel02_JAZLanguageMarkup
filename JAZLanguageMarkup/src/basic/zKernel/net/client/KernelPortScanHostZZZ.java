...
public class KernelPortScanHostZZZ  extends AbstractKernelUseObjectZZZ{
	...
	public boolean scan(int iPortRangeLowestIn, int iPortRangeHighestIn) throws ExceptionZZZ{
		...
		//Damit wird ein vorheriger Port-Scan wieder "gel�scht"
			...
			do{				
				...
				try{
					if(bStored==false) {
						...
						Thread.sleep(10);   //Damit schl�ft DIESER Thread und nicht th
					}					
				}catch(InterruptedException ex){					
				}				
			}while(bStored==false);
			th.start();  //Der Thread f�llt die Arraylist der Portscans
			try{
				Thread.sleep(1);
			}catch(InterruptedException ex){					
			}		
			...
			for(int icount = 0; icount <= iStorageSize-1; icount ++){
				...
				if (objRunner!=null){
					if(objRunner.getFlag("IsUnknownHost")){
						...
						break; // es reicht ein runner-objekt auf den "UnknownHost" Fehler zu pr�fen.
					}
				}
			}//END for
